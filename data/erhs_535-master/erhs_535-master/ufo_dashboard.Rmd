...
ufo_sightings_co <- ufo_sightings %>% 
  ...
  filter(state == 'co') %>% 
  filter(sighting_year >= 1968 & sighting_year <= 1972)
...
leaflet() %>% 
  ...
  addCircleMarkers(ufo_sightings_co$longitude, 
                   ...
                   fillOpacity = 0.2,
                   opacity = 0.6,
                   popup = paste(ufo_sightings_co$description,
                                 ufo_sightings_co$encounter_length, 
                                 sep = ",")) 
...
### Encounter Lenght in Colorado by Year

```{r}
ufo_sightings_co %>% 
  group_by(sighting_year) %>% 
  summarize(length_year = sum(encounter_length)) %>% 
  ungroup() %>% 
  ggplot() +
  geom_line(aes(x = sighting_year, y = length_year)) +
  labs(title = "UFO Sighting Time in Colorado per Year", x = "Year", y = "Cumulative Sighting Time") +
  theme_few()
